...
class CategoryControllerTest    {
    ...
    public void testSuccessfulladdcategory() throws Exception {
        CategoryCreateDto categoryCreateDto = CategoryCreateDto.builder()
                ...
                .name("name")
                .build();
        given(categoryServiceImpl.createCategory(categoryCreateDto)).willReturn(new Category(1L, "name", "desc"));
    ...
    public void getAllCategory() throws Exception {
        List<Category> categoryList = Arrays.asList(new Category(1L, "name", "desc"),
                new Category(2L, "name1", "desc1"));
        given(categoryServiceImpl.getAllCategories()).willReturn(categoryList);
        List<CategoryDto> expected = CategoryDto.from(categoryList);
    ...
    public void getOneCategory() throws Exception{
        given(categoryServiceImpl.getCategory(1L)).willReturn(new Category(1L, "name", "desc"));
        MvcResult result = mockMvc.perform(get("/categories/1")).andReturn();
        CategoryDto expected = CategoryDto.from(new Category(1L, "name", "desc"));
        JSONAssert.assertEquals(objectMapper.writeValueAsString(expected), result.getResponse().getContentAsString(), false);
    }
    @Test
    public void getOneCategoryIfNotExists() throws Exception{
        given(categoryServiceImpl.getCategory(1L)).willThrow(new NotFoundException(Category.class, "id", String.valueOf(1L)));
    ...
    public void deleteCategoryIfNotExists() throws Exception{
        doThrow(new NotFoundException(Category.class, "id", String.valueOf(111L))).when(categoryServiceImpl).deleteCategory(111L);
    ...
    public void updateCategoryIfNotExists() throws Exception{
        CategoryDto categoryDto = CategoryDto.builder()
                ...
                .name("name")
                .build();
        doThrow(new NotFoundException(Category.class, "id", String.valueOf(111L))).when(categoryServiceImpl).updateCategory(categoryDto);
    ...
    public void updateCategoryIfExists() throws Exception{
        CategoryDto categoryDto = CategoryDto.builder()
                ...
                .name("name")
                .build();
        Category updated = new Category(115L, "name", "desc");
        given(categoryServiceImpl.updateCategory(categoryDto)).willReturn(updated);
        MvcResult result = mockMvc.perform(put("/categories/update")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(categoryDto))).andReturn();
        JSONAssert.assertEquals(objectMapper.writeValueAsString(updated), result.getResponse().getContentAsString(), false);
    }
